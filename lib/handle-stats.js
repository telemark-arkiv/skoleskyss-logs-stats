...
module.exports = query => {
  return new Promise((resolve, reject) => {
    if (query.action === 'total') {
      ...
      logs.aggregate([{'$project': {'documentId': '$_id', 'documentStatus': '$documentStatus'}},
        {'$unwind': '$documentStatus'},
        {'$group': {'_id': '$documentId', 'documentStatus': {'$addToSet': '$documentStatus.status'}}},
        {'$unwind': '$documentStatus'},
        {'$group': {'_id': '$documentStatus', 'total': {'$sum': 1}}}])
        .sort({'total': -1}, (error, data) => {
          if (error) {
